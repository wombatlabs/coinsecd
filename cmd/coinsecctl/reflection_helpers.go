...
// 2. A wrapper that implements isCoinsecdMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `CoinsecdMessage_` prefix and `Request` suffix
...
// generateCoinsecdMessage generates a wrapped CoinsecdMessage with the given `commandValue`
func generateCoinsecdMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.CoinsecdMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	coinsecdMessage := reflect.New(reflect.TypeOf(protowire.CoinsecdMessage{}))
	coinsecdMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return coinsecdMessage.Interface().(*protowire.CoinsecdMessage), nil
}
