...
// FormatSec takes the amount of sompis as uint64, and returns amount of SEC with 9 decimal places
func FormatSec(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.9f", float64(amount)/constants.SompiPerCoinsec)
	}
	return res
}
