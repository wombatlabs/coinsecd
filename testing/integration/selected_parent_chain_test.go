...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of coinsecd instances
	coinsecd1, coinsecd2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := coinsecd1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In coinsecd2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, coinsecd2)
	}

	// Connect the two coinsecds. This should trigger sync
	// between the two nodes
	connect(t, coinsecd1, coinsecd2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := coinsecd1.rpcClient.GetVirtualSelectedParentChainFromBlock(
