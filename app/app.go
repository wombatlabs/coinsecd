...
var serviceDescription = &winservice.ServiceDescription{
	Name:        "coinsecdsvc",
	DisplayName: "Coinsecd Service",
	Description: "Downloads and stays synchronized with the Coinsec blockDAG and " +
		"provides DAG services to applications.",
}

type coinsecdApp struct {
	cfg *config.Config
}

// StartApp starts the coinsecd app, and blocks until it finishes running
func StartApp() error {
...
func (app *coinsecdApp) main(startedChan chan<- struct{}) error {
	...
	if err != nil {
		log.Errorf("Unable to start coinsecd: %+v", err)
		return err
	}

	defer func() {
		log.Infof("Gracefully shutting down coinsecd...")
		...
		log.Infof("Coinsecd shutdown complete")
	}()
