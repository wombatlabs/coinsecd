/*
Package appmessage implements the coinsec appmessage protocol.

At a high level, this package provides support for marshalling and unmarshalling
supported coinsec messages to and from the appmessage. This package does not deal
...
which coinsec network it is a part of, its type, how big it is, and a checksum
...
of the currently supported coinsec messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
appmessage using coinsec encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the coinsec messages are intended
...
and write coinsec messages. The following sections provide a quick overview of
...
As discussed in the coinsec message overview section, this package reads
and writes coinsec messages using a generic interface named Message. In
...
In order to unmarshall coinsec messages from the appmessage, use the ReadMessage
function. It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a coinsec peer. Example syntax is:
	...
	msg, rawPayload, err := appmessage.ReadMessage(conn, pver, coinsecNet)
	if err != nil {
...
In order to marshall coinsec messages to the appmessage, use the WriteMessage
function. It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a coinsec peer. Example syntax to request addresses
from a remote peer is:
	...
	// error.
	err := appmessage.WriteMessage(conn, msg, pver, coinsecNet)
	if err != nil {
		// Log and handle the error
	}
