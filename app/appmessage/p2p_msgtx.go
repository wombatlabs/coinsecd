...
// Outpoint defines a coinsec data type that is used to track previous
// transaction outputs.
...
// NewOutpoint returns a new coinsec transaction outpoint point with the
...
// TxIn defines a coinsec transaction input.
type TxIn struct {
...
// NewTxIn returns a new coinsec transaction input with the provided
...
// TxOut defines a coinsec transaction output.
type TxOut struct {
	Value        uint64
	ScriptPubKey *externalapi.ScriptPublicKey
}

// NewTxOut returns a new coinsec transaction output with the provided
...
// MsgTx implements the Message interface and represents a coinsec tx message.
