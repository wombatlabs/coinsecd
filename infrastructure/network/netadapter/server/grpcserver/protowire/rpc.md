...
Clients are expected to build RequestMessages and wrap them in CoinsecdMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a CoinsecdMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a CoinsecdMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network coinsecd is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which coinsec address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether coinsecd thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when coinsecd is not synced. That is because when coinsecd isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known coinsecd addresses in the
...
currently connected to this coinsecd.
...
| isOutbound | [bool](#bool) |  | Whether this coinsecd initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this coinsecd |
...
AddPeerRequestMessage adds a peer to coinsecd&#39;s outgoing connection list.
This will, in most cases, result in coinsecd connecting to said peer.
...
parent chain from some startHash to this coinsecd&#39;s current virtual
...
GetBlockCountRequestMessage requests the current number of blocks in this coinsecd.
...
ShutDownRequestMessage shuts down this coinsecd.
...
This call is only available when this coinsecd was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given coinsecd addresses
