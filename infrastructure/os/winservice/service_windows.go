...
// long-running coinsecdMain (which is the real meat of coinsecd), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start coinsecdMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once coinsecd is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("coinsecdMain-windows", func() {
...
// logServiceStart logs information about coinsecd when the main server has
